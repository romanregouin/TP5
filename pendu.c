...
int main(int nargs, char** args) {
	...
	int random,try,gagner;
	char* deviner=malloc(Max);
  // initlalize the random generator
	srand(clock());
 	char* d[10]={"savon","papillon","xilophone","tulukmatou","nathan","rigolade","saumon","pecule","yo","kayak"};
	try=0;
	printf(" 	Bonjour est bienvenue dans le jeu du Pendue !\n 	Vous avez 10 coups pour deviner le mot !\n 	Bonne chance !\n");
	random=rand()%10;
	...
	while(continuer){
		...
		if(try<=10){
			printf(" 	Bravo, vous avez gagné !!!!  \n 	Pour relancer une partie taper 'oui' sinon taper 'non'\n");
		}else{
			 printf("	 Dommage vous avez dépassé les 10 essaies ! C'est perdu pour cette fois !\n         Pour relancer une partie taper 'oui' sinon taper 'non'\n");
		}
		do{
			...
			if(strcmp(reponse2,"non")==0){
				...
				printf("        Il faut taper 'oui' pour relancer ou taper 'non' pour arreter.\n");
			}
